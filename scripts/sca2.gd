...
func unrewrite(s: String, rev: bool):
	...
	for rule in rewrite_rules_array:
		if rule.length() > 2 and find(rule, "|") != -1:
			var parse = rule.split("|")
			var regex = RegEx.new
			regex.compile(parse[p1])
			ret = regex.sub(ret, parse[p2], true)
			ret = ret.replace(regex, parse[p2]);
	return ret
	...
	while j < env.length():
		match (env[j]):
			...
			'…':
				...
				var newenv = env.substr(j + 1, env.length() - (2 * (j - 1)))
...
func apply_rule(inword: String, r: int) -> String:
	...
	while i <= inword.length() and inword[i] != '‣':
		if _match(inword, i, thisrule[0], thisrule[2]):
			var tgt = thisrule[0]
			var repl = thisrule[1]
			...
			outword += inword.substr(gix, inword.length() - 2 * gix)
...
func transform(inword: String) -> String:
	if inword.length() > 0:
